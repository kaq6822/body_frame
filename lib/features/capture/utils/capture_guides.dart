...
List<String> captureGuideMessages(BodyDirection direction) {
  ...
  const height = '카메라 높이: 회원의 가슴 높이에 맞춰 수평으로 두세요.';
  const foot = '발 위치: 격자 중앙 세로선에 두 발 사이를 맞춰 서주세요.';

  switch (direction) {
    case BodyDirection.front:
      return ['회원 위치: 카메라를 정면으로 바라보고 서주세요.', foot, height, distance];
    case BodyDirection.leftSide:
      return ['회원 위치: 왼쪽 옆모습이 카메라를 향하도록 서주세요.', foot, height, distance];
    case BodyDirection.rightSide:
      return ['회원 위치: 오른쪽 옆모습이 카메라를 향하도록 서주세요.', foot, height, distance];
    case BodyDirection.back:
      return ['회원 위치: 뒷모습이 보이도록 카메라 반대쪽을 바라봐 주세요.', foot, height, distance];
