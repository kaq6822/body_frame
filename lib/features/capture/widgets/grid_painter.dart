...
class GridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final color = Color(settings.colorValue).withValues(
    ...
    for (var dx = spacing; centerX + dx <= size.width || centerX - dx >= 0; dx += spacing) {
      if (centerX + dx <= size.width) {
        canvas.drawLine(Offset(centerX + dx, 0), Offset(centerX + dx, size.height), linePaint);
      }
      if (centerX - dx >= 0) {
        canvas.drawLine(Offset(centerX - dx, 0), Offset(centerX - dx, size.height), linePaint);
      }
    ...
    for (var dy = spacing; centerY + dy <= size.height || centerY - dy >= 0; dy += spacing) {
      if (centerY + dy <= size.height) {
        canvas.drawLine(Offset(0, centerY + dy), Offset(size.width, centerY + dy), linePaint);
      }
      if (centerY - dy >= 0) {
        canvas.drawLine(Offset(0, centerY - dy), Offset(size.width, centerY - dy), linePaint);
      }
    ...
    final emphasisPaint = Paint()
      ...
      ..style = PaintingStyle.stroke;
    canvas.drawLine(Offset(centerX, 0), Offset(centerX, size.height), emphasisPaint);

    // 중앙 기준점.
    canvas.drawCircle(Offset(centerX, centerY), math.max(settings.lineWidth * 2, 3), centerPaint);
  }

  void _drawDashedVerticalLine(Canvas canvas, double x, double height, Paint paint) {
  ...
  @override
  bool shouldRepaint(covariant GridPainter oldDelegate) => oldDelegate.settings != settings;
}
